...
export const useNestedPagerView = (pagerViewProps: { pagesAmount: number }) => {
  ...
  const proceedToNext = useCallback(() => {
    if (rest.activePage < rest.pages.length - 1) {
      rest.setPage(rest.activePage + 1);
    }
  }, [rest.activePage, rest.pages.length, rest.setPage]);
  ...
  const isLastPage = useMemo(() => {
    return rest.activePage === rest.pages.length - 1;
  }, [rest.activePage, rest.pages.length]);
